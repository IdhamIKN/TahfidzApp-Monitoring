...
class AssessmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            } elseif ($this->getUserLogin()->account_type == User::ACCOUNT_TYPE_PARENT) {
                $siswaIds = SiswaHasParent::where('parent_id', $this->getUserLogin()->id)->pluck('siswa_id');
                $data = Siswa::whereIn('id', $siswaIds)->get(['id', 'siswa_name', 'memorization_type', 'class_id']);
            } else {
                $data = Siswa::all();
            }
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    if ($this->getUserLogin()->account_type == User::ACCOUNT_TYPE_PARENT) {
                        $btn = '<button name="btnAssessment" onclick="btnAss(' . $row->id . ')" type="button" class="btn btn-info"><span class="glyphicon glyphicon-tasks"></span></button>';
                        $pass = '<button name="btnAssessment1" onclick="btnAss1(' . $row->id . ')" type="button" class="btn btn-info"><span class="glyphicon glyphicon-calendar"></span></button>';
                         return $btn . '&nbsp' . $pass ;
                    } else {
                        $pass = '<button type="hidden" name="btnAssessment" onclick="btnAss(' . $row->id . ')" type="button" class="btn btn-info"><span class="glyphicon glyphicon-calendar"></span></button>';
    ...
    public function assessment($id_siswa, Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = AssessmentLog::where('siswa_id', $id_siswa)
        ...
        $absen = SiswaHasAbsensi::where('siswa_id', $id_siswa)->get();
    ...
    public function absensi($id_siswa, Request $request)
    {
        $data_siswa = Siswa::findOrFail($id_siswa);
        ...
        if ($this->getUserPermission('create assessment')) {
            if ($data_siswa->memorization_type != Siswa::TYPE_IQRO) {
                ...
                $this->systemLog(false, 'Mengakses Halaman Assessment');
                return view('assessment.hafalan', [
                    'active' => 'assessment',
                    'data_siswa' => $data_siswa,
                    'absen' => $absen,
                    'Attendance' => $Attendance
    ...
    public function pay(Request $request)
    {
        ...
        return view('error.unauthorized', ['active' => 'pembayaran']);
            // If the processing is successful, send a success notification
    // $request->session()->flash('success', 'Berhasil mengakses rute /pay');

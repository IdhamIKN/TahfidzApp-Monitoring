...
class SiswaController extends Controller
{
    ...
    //     // Generate NIS based on year and registration number
    //     $year = date('Y');
    //     $reg_num = Siswa::where('class_id', $request->get('class_id'))->count() + 1;
    //     $nis = $year . $request->get('memorization_type') . sprintf('%02d', $reg_num);
    ...
    //     // Validate if the student has not been entered before
    //     if(Siswa::validateSiswa($request->get('class_id'),$request->get('siswa_name')))
    //     {
    //         DB::rollBack();
    //         return redirect('siswa')->with('alert_error', 'Data siswa telah dimasukkan sebelumnya');
    ...
    public function store(StoreSiswaRequest $request)
    {
        ...
        $last_nis = Siswa::where('nis', 'like', date('Y') . '%')
            ->orderBy('nis', 'desc')
            ->first();
        $reg_num = ($last_nis) ? intval(substr($last_nis->nis, -2)) + 1 : 1;
        $nis = date('Y') . $request->get('memorization_type') . sprintf('%02d', $reg_num);

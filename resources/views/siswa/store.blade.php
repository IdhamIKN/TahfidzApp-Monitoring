...
@section('content')
	...
	<form method="post" action="{{ route('store-siswa') }}">
		...
		<div class="form-group">
			...
			<input type="text" class="form-control" value="{{ old('') }}" name="siswa_name">
        ...
        <div class="form-group">
            <label>Jenis Hafalan</label>
            <select class="form-control" name="memorization_type">
                <option value="{{ Siswa::TYPE_IQRO }}" @if(old('memorization_type') == Siswa::TYPE_IQRO) selected @endif >Iqro</option>
                <option value="{{ Siswa::TYPE_QURAN }}" @if(old('memorization_type') == Siswa::TYPE_QURAN) selected @endif >Alquran</option>
            </select>
            @if ($errors->has('memorization_type'))
                <div class="error"><p style="color: red"><span>&#42;</span> {{ $errors->first('memorization_type') }}</p></div>
            @endif
